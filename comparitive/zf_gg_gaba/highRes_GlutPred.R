...
Gg_adult_exc <- subset(Gg_adult, subset = tg_class == "Excitatory neurons" & anno_level_1 != "Ex_Pre")
Idents(Gg_adult_exc) <- "anno_level_1"
...
predictions_join_ann1 <- TransferData(
  anchorset = anchors_GGtoZF, refdata = Gg_adult_exc$anno_level_1,
  weight.reduction = "cca"
)
seuObj_glut <- AddMetaData(seuObj_glut, metadata = predictions_join_ann1)
...
metaComb <- full_join(
  ...
  rownames_to_column(predictions_join_ann1, var = "rowname"),
  by = "rowname"
)
...
heatToPlot <- heatToPlot[, order(factor(colnames(heatToPlot), levels = gg_levels_lvl1_v2))] # gg_levels_lvl3_v2

# glut_corr_heatmap <- pheatmap(heatToPlot,
#                             cellwidth = 15, cellheight = 15,
#                             cluster_rows = F, cluster_cols = F,
#                             color = colorRampPalette(c("white", "#aa7c00"))(100)) # colorRampPalette(c("white", "#aa7c00"))(100)) / viridis(100, option = "viridis") / rev(viridis(100, option = "mako"))
#
# glut_corr_heatmap
...
res <- run_label_transfer_shuffle_heatmap(heatToPlot,
  ...
  RDSfiles = RDSfiles,
  rds_name = "lblTrnf_Gluts_lvl1_300shuff.rds"
)

p1 <- res$plot

ggsave(file.path(Figures_dir, "corrplot_lablTrns_annoLvl1_wSig.svg"),
  plot = p1,
  width = 7, height = 8
)
...
if (redoMarkers) {
  ...
  saveRDS(ggGlut_Markers, paste0(RDSfiles, "/DEGs/ggGlutMarkers_annLvl1_0.2pct-pctDiff0.3-0.1LogFC_20251027.rds"))
...
ggGlut_Markers <- readRDS(paste0(RDSfiles, "/DEGs/ggGlutMarkers_annLvl1_0.2pct-pctDiff0.3-0.1LogFC_20251027.rds")) %>%
...
GSI_heatmap <- generate_SpearmanCorrHeatmap_wSig(
  ...
  other.levels = gg.levels.lvl1.v2,
  minSetTo = 0,
...
ggsave(file.path(Figures_dir, "corrplot_gsi_annoLvl1.svg"),
  plot = p2,
  width = 10, height = 9
)
...
ggsave(file.path(Figures_dir, "corrplot_combined_annoLvl1_w2sig.svg"),
  plot = p3,
  width = 10, height = 9
)
